...
이 부분에서 제공하는 규칙들은 웹 브라우저의 동일 사이트 정책(same origin policy) 에 따라, 제한된 공간에서 실행되는 자바스크립트 기반 클라이언트를 지원하기 위한
REST API에 적용된다. 동일 사이트 정책은 동일 도메인 원칙이 라고도 하며, 웹 브라우저 기반의 자바스크립트 클라이언트에서 
