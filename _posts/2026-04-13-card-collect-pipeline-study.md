...
##### Phase 1 — temp 순회, 기존 건 매칭

각 temp 레코드에 대해 olds 를 순회하며 매칭을 시도한다. **매칭 기준은 자연 키**:

```
approvalNum + approvalDT(날짜만, 시간 제외) + approvalAmount + approvalType
```
...
| 조건 | CompareResult | 처리 |
|------|---------------|------|
| 매칭 없음 | `NEW` | 본 테이블 INSERT |
| 매칭 + 완전 동일 | `DO_NOTHING` | **아무 작업 안 함** (불필요한 UPDATE 방지) |
| 매칭 + 승인일시 시간만 다름 | `UPDATED` | 본 테이블 UPDATE, 기존 휴폐업 정보 보존 |
| 매칭 + `storeBizId` 변경 | `UPDATED_STORE_BIZ_ID` | UPDATE + 가맹점 과세유형 재조회 필요 |
| 매칭 + `storeCompanyType` 이 NULL | `STORE_TAX_TYPE_IS_NULL` | 데이터는 동일, 과세유형만 보강 |
| 매칭 + 그 외 필드 변경 | `UPDATED` | 본 테이블 UPDATE |

중요: 매칭된 olds 레코드는 **즉시 olds 리스트에서 제거** 된다. 이게 Phase 2 의 복선.
