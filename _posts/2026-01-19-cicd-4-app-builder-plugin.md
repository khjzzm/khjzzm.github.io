...
1. [프로젝트 개요](#1-프로젝트-개요)
2. [전체 CI/CD 흐름](#2-전체-cicd-흐름)
3. [Jenkins 연동 - Shared Library](#3-jenkins-연동---shared-library)
4. [Gradle 플러그인 구조](#4-gradle-플러그인-구조)
5. [데이터 모델 - Entity](#5-데이터-모델---entity)
6. [모듈 감지 - GetTargetsTask](#6-모듈-감지---gettargetstask)
7. [빌드 실행 - AppBuilderTask](#7-빌드-실행---appbuildertask)
8. [3가지 Executor](#8-3가지-executor)
9. [학습 로드맵](#9-학습-로드맵)
...
**다른 프로젝트들(order, payment 등)을 빌드하기 위한 CI/CD 자동화 도구**다.
...
이 플러그인의 핵심 기능은 **멀티 모듈 프로젝트에서 변경된 모듈만 선택적으로 빌드**하는 것이다.
...
**요약:** `@Library`로 호출하는 것은 **Jenkins Shared Library**이고, 이 Library가 내부에서 **Gradle Plugin**을 호출한다. 사용자는 Library만 호출하면 된다.
...
**참고:** GitHub이 Jenkinsfile을 직접 실행하는 게 아니라, GitHub은 "변경됐다"고 알려주기만 하고, **Jenkins가 소스를 가져와서 Jenkinsfile을 읽고 실행**한다.
...
이 프로젝트는 **두 개의 진입점**이 있다:

| 구분          | 진입점                    | 파일 위치                                                   |
|-------------|------------------------|---------------------------------------------------------|
| **Jenkins** | `@Library` → `build()` | `app-builder-gradle-plugin-pipeline/vars/build.groovy`  |
...
이 파일은 Jenkins Shared Library의 **진입점**으로, 전체 CI/CD 파이프라인을 오케스트레이션한다.
...
| `ECR_REGISTRY`, `helmValuesRepo` 등 | 사용자 프로젝트 `build.gradle.kts`에서 직접 설정 |
...
데이터 모델은 **플러그인 자체의 정보가 아니라, 플러그인을 호출한 프로젝트의 정보**를 담는다.
...
GetTargetsTask 코드에서 `project`는 **Gradle의 핵심 객체**다:

```kotlin
val executor = DetectorExecutor(project, logger)
```

`org.gradle.api.Project`는 Gradle이 빌드할 때 자동으로 주입하는 객체로, 현재 빌드 중인 프로젝트의 모든 정보에 접근할 수 있다.
...
**중요**: 이 플러그인에서 `project`는 **app-builder-plugin 프로젝트**를 가리킨다. 빌드 대상인 order, payment 프로젝트가 아니다!
...
**의존성이 있는 작업들을 올바른 순서로 정렬**하는 알고리즘이다.
...
각 Executor는 **단일 책임 원칙**에 따라 분리되어 있다.
